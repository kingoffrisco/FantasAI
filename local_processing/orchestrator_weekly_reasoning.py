...
Default --limit is 300 (bumped from 15 on 2026-08-22 after validating output
quality on smaller batches). At the ~2s/player pace seen once the model is
warm, 300 players is ~10 minutes of actual generation — but the registered
task's ExecutionTimeLimit should be generous (see docs/WEEKLY_DEEP_REASONING_TASK.md)
since per-player time spikes hard (seen up to ~2 min/player) whenever
something else on the box evicts the 30B model from VRAM mid-run.
...
Registration command:
    See docs/WEEKLY_DEEP_REASONING_TASK.md for the exact PowerShell registration block.

Usage:
  python orchestrator_weekly_reasoning.py
  python orchestrator_weekly_reasoning.py --limit 300

  ...
  async fetch(request, env) {
    ...
    try {
      ...
      if (method === 'POST') {
        ...
        // League management — requires X-FantasAI-Key (was open, an unauthenticated
        // request used it to create an unexplained league)
        ...
        if (url.pathname === '/api/v1/login-log')             return await handleLoginLogPost(request, env);
...
// ── Admin Login Log ──────────────────────────────────────────────────────────
// Shape: [{ email, teamId, teamName, isAdmin, ts }, ...], most recent last.

const LOGIN_LOG_KEY = 'fantasai/admin/login_log.json';
const LOGIN_LOG_MAX_ENTRIES = 1000; // keep most recent N, drop older

async function handleLoginLogPost(request, env) {
  const body = await request.json().catch(() => ({}));
  const { email, teamId, teamName, isAdmin } = body;
  if (!email) return json({ error: 'email required' }, 400);

  let entries = [];
  try {
    const obj = await env.BUCKET.get(LOGIN_LOG_KEY);
    if (obj) entries = JSON.parse(await obj.text());
  } catch (_) {}
  ...
  if (entries.length > LOGIN_LOG_MAX_ENTRIES) entries = entries.slice(-LOGIN_LOG_MAX_ENTRIES);

  await env.BUCKET.put(LOGIN_LOG_KEY, JSON.stringify(entries), { httpMetadata: { contentType: 'application/json' } });
  return json({ status: 'ok' }, 200);
}

// Registered via PROTECTED_GET, which already gates on X-FantasAI-Key before
// calling the handler — this route exposes every user's email + login history.
async function handleLoginLogGet(url, env, request) {
  try {
    const obj = await env.BUCKET.get(LOGIN_LOG_KEY);
    if (!obj) return { status: 'ok', entries: [] };
...
async function handleLeagueCreate(request, env) {
  // Locked down after an unauthenticated request created an unexplained league in R2.
  const k = request.headers.get('X-FantasAI-Key');
  if (!env.FANTASAI_KEY || k !== env.FANTASAI_KEY) return json({ error: 'Unauthorized' }, 401);

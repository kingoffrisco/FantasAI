...
import LoginLog from './screens/LoginLog.jsx';
...
const CRUMBS = {
  ...
  'admin-loginlog': ['Admin', 'Login Log'],
...
export default function App() {
  ...
  function handleLogin(u) {
    ...
    api.loginLog.record({ email: u.email, teamId: u.teamId, teamName: u.teamName, isAdmin: u.isAdmin });
    ...
    <React.Fragment>
      ...
      <div className={shellClass} style={shellLayoutStyle}>
        ...
        <div className="main" style={{ zoom: appFontSize / 12 }}>
          ...
          {active === 'admin-loginlog' && <LoginLog />}
          {active === 'admin-leagues' && <AdminLeagues />}
        ...
        {/* Return-to-Draft floating banner — shown while a draft is actively running, not after completion */}
        {draftInProgress && active !== 'draft' && !draftMeta?.draftComplete && (() => {
          const isMock      = draftInProgress === 'mock';
          const isPaused    = draftMeta?.draftPaused;
          const accent      = isPaused ? '#ff5a6e' : isMock ? '#ffb547' : 'var(--accent-2)';
          ...
          return (
            <div style={{
              ...
              background: isPaused ? 'rgba(40,8,12,.97)' : isMock ? 'rgba(30,24,0,.97)' : 'rgba(0,20,40,.97)',
              border: `1.5px solid ${accent}`,
              borderRadius: 16, padding: '12px 20px',
              boxShadow: `0 8px 40px rgba(0,0,0,.7), 0 0 28px ${isPaused ? 'rgba(255,90,110,.35)' : isMock ? 'rgba(255,181,71,.3)' : 'rgba(78,168,255,.3)'}`,
              cursor: 'pointer', minWidth: 340,
              animation: isPaused ? 'blink 1s ease-in-out infinite' : 'none',
            }} onClick={() => setActive('draft')}>
              {/* Status dot */}
              <div style={{ width: 12, height: 12, borderRadius: '50%', background: accent, boxShadow: `0 0 10px ${accent}`, animation: 'blink 0.8s infinite', flexShrink: 0 }} />

              <div style={{ flex: 1, minWidth: 0 }}>
                {/* Top row */}
                <div style={{ fontSize: 11, fontWeight: 800, color: accent, letterSpacing: '.08em', textTransform: 'uppercase', lineHeight: 1 }}>
                  {isPaused ? '⏸ DRAFT PAUSED' : `${isMock ? '🏈 Mock Draft' : '🏈 Live Draft'} In Progress · Pick #${currentPickNum}`}
                </div>
                {/* Bottom row */}
                {isPaused ? (
              ...
              <button
                style={{ background: '#1affa0', color: '#000', border: 'none', borderRadius: 10, padding: '10px 22px', fontSize: 13, fontWeight: 900, cursor: 'pointer', flexShrink: 0, letterSpacing: '.05em', whiteSpace: 'nowrap', boxShadow: '0 2px 12px rgba(26,255,160,.3)' }}

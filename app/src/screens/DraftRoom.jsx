...
import { LEAGUE_TEAMS, DRAFT_PICKS, TEAMS_ORDER, DRAFT_ROUNDS, QUEUE as INIT_QUEUE, ROSTER_CONFIG, findTeam } from '../lib/data.js';
...
export default function DraftRoom({ aiMode, user, onNav, onDraftPick, onDraftComplete, onDraftStatusChange, registerExitMock, onOpenPlayer, showMobile = false, draftPriorityIds }) {
  ...
  const [activeScheduledMockId, setActiveScheduledMockId] = React.useState(null);
  ...
  function exitMockDraft() {
    ...
    setMockSlotIndex(null);
    setMockTeamsOrder([...TEAMS_ORDER]);
    // If the host exits an active scheduled mock, terminate it so nobody can join it anymore.
    setMockSchedule(prev => prev.filter(d => {
      const isActive = activeScheduledMockId ? d.id === activeScheduledMockId : d.status === 'in_progress';
      return !(isActive && d.hostTeamId === myTeamId);
    }));
    setActiveScheduledMockId(null);
    setPaused(false);
  ...
  const [draftStateHydrated, setDraftStateHydrated] = React.useState(false);
  const [queue, setQueue]     = React.useState(() => {
    const teamId = user?.teamId || 1;
    try {
      const saved = JSON.parse(localStorage.getItem(`fantasai_draft_queue_${teamId}`) || 'null');
      return Array.isArray(saved) ? saved : INIT_QUEUE;
    } catch {
      return INIT_QUEUE;
    }
  });
  ...
  const [ultraCompactTeamsView, setUltraCompactTeamsView] = React.useState(() => {
    try { return JSON.parse(localStorage.getItem('fantasai_ultra_compact_teams_view') || 'false'); } catch { return false; }
  });
  ...
  const [chatMessages, setChatMessages] = React.useState(() => {
    try {
      const saved = JSON.parse(localStorage.getItem('fantasai_draft_chat_messages') || '[]');
  ...
  const [layoutSizes, setLayoutSizes] = React.useState(() => {
    try {
      const saved = JSON.parse(localStorage.getItem('fantasai_draft_layout_sizes') || 'null');
      if (saved && Number.isFinite(saved.rightCol)) {
        return {
          rightCol: Math.round(saved.rightCol),
        };
      }
    } catch {}
    return { rightCol: 620 };
  ...
  React.useEffect(() => {
    try { localStorage.setItem('fantasai_draft_layout_sizes', JSON.stringify(layoutSizes)); } catch {}
  }, [layoutSizes]);

  React.useEffect(() => {
    try { localStorage.setItem('fantasai_ultra_compact_teams_view', JSON.stringify(ultraCompactTeamsView)); } catch {}
  }, [ultraCompactTeamsView]);
  ...
  React.useEffect(() => {
    function onMove(e) {
      ...
      if (!mode || !draftGridRef.current) return;
      const rect = draftGridRef.current.getBoundingClientRect();
      if (mode === 'col') {
        const minBoard = 560;
        const minRight = 460;
        const maxRight = Math.max(minRight, rect.width - minBoard);
        const rightCol = clamp(rect.right - e.clientX, minRight, maxRight);
        setLayoutSizes(prev => ({ ...prev, rightCol: Math.round(rightCol) }));
      }
    }
  ...
  const myTeamName = (LEAGUE_TEAMS.find(t => t.id === (user?.teamId || 1))?.name) || user?.name || 'You';
  const myName = myTeamName;

  function hydrateDraftPicks(partialPicks) {
    const byPick = new Map(
      (Array.isArray(partialPicks) ? partialPicks : [])
        .filter(p => p && Number.isFinite(Number(p.pickNum)))
        .map(p => [Number(p.pickNum), p])
    );
    return DRAFT_PICKS.map(base => {
      const src = byPick.get(Number(base.pickNum));
      return src ? { ...base, ...src, pickNum: base.pickNum } : { ...base };
    });
  }
  ...
  function mergeChatMessages(remoteMessages, localMessages) {
    const normalizeMessage = (m) => {
      ...
      const teamIdNum = Number(m.teamId);
      const teamId = Number.isFinite(teamIdNum) ? teamIdNum : null;
      const team = teamId ? LEAGUE_TEAMS.find(t => t.id === teamId) : null;
      return {
        ...m,
        teamId,
        who: team?.name || m.who || 'Team',
      };
    };

    const map = new Map();
    const all = [...(Array.isArray(localMessages) ? localMessages : []), ...(Array.isArray(remoteMessages) ? remoteMessages : [])];
    for (const raw of all) {
      const m = normalizeMessage(raw);
      if (!m) continue;
      const key = m.id || `${m.at || ''}|${m.ts || ''}|${m.who || ''}|${m.msg || ''}`;
      if (!map.has(key)) map.set(key, m);
    }
    const merged = [...map.values()];
    merged.sort((a, b) => (Number(b.at) || 0) - (Number(a.at) || 0));
    return merged;
  }

  function firstOpenPickNum(picks) {
    const next = (picks || []).find(p => !p.playerId);
    return next?.pickNum ?? (DRAFT_PICKS.length + 1);
  }

  function postChat(msg, extra = {}) {
    // Chat should contain only user-entered messages, not synthetic/system inserts.
    return;
  ...
  const pullSharedDraftState = React.useCallback(async ({ manual = false } = {}) => {
    if (mockActive) return;
    if (manual) setSyncActive(true);
    try {
      const [picksRes, chatRes, queueRes, stateRes] = await Promise.allSettled([
        api.draftPicks.get(),
        api.draftChat.get(),
        api.draftQueue.get(),
        api.draftState.get(),
      ...
      const remote = picksRes.status === 'fulfilled' ? picksRes.value : null;
      if (Array.isArray(remote)) {
        anySuccess = true;
        const remoteHydrated = hydrateDraftPicks(remote);
        setLivePicks(prev => {
          const merged = mergeDraftPicks(remoteHydrated, prev);
          const same = JSON.stringify(merged) === JSON.stringify(prev);
          return same ? prev : merged;
        });
      }

      const remoteChatRaw = chatRes.status === 'fulfilled' ? chatRes.value : null;
      if (Array.isArray(remoteChatRaw)) {
        anySuccess = true;
        setChatMessages(prev => {
          const merged = mergeChatMessages(remoteChatRaw, prev);
          const same = JSON.stringify(merged) === JSON.stringify(prev);
          return same ? prev : merged;
        });
      }

      const remoteQueueRaw = queueRes.status === 'fulfilled' ? queueRes.value : null;
      if (remoteQueueRaw && typeof remoteQueueRaw === 'object') {
        anySuccess = true;
        const myTeamId = user?.teamId || 1;
        const remoteQueue = Array.isArray(remoteQueueRaw?.[String(myTeamId)])
          ? remoteQueueRaw[String(myTeamId)]
          : [];
        setQueue(prev => {
          const same = JSON.stringify(prev) === JSON.stringify(remoteQueue);
          return same ? prev : remoteQueue;
        });
      }

      const remoteDraftState = stateRes.status === 'fulfilled' ? stateRes.value : null;
      if (remoteDraftState && typeof remoteDraftState === 'object') {
        anySuccess = true;
        if (typeof remoteDraftState.paused === 'boolean') {
          setPaused(prev => prev === remoteDraftState.paused ? prev : remoteDraftState.paused);
        }
        if (Number.isFinite(Number(remoteDraftState.clockSeconds))) {
          const nextClock = Number(remoteDraftState.clockSeconds);
          setClockSeconds(prev => prev === nextClock ? prev : nextClock);
        }
        if (Number.isFinite(Number(remoteDraftState.seconds))) {
          const nextSeconds = Math.max(0, Number(remoteDraftState.seconds));
      ...
      if (manual) setSyncActive(false);
    }
  }, [mockActive, user?.teamId]);

  // Persist live draft state so navigation away and back doesn't reset picks
  React.useEffect(() => {
    if (!mockActive && draftStateHydrated) {
      try { localStorage.setItem('fantasai_live_picks', JSON.stringify(livePicks)); } catch {}
      // Sync the full array so clears/resets also propagate across devices.
      api.draftPicks.save(livePicks);
    }
  }, [livePicks, mockActive, draftStateHydrated]);

  React.useEffect(() => {
    if (mockActive || !draftStateHydrated) return;
    try { localStorage.setItem('fantasai_draft_chat_messages', JSON.stringify(chatMessages)); } catch {}
    let cancelled = false;
    (async () => {
      try {
        const remote = await api.draftChat.get();
        if (cancelled) return;
        const merged = mergeChatMessages(Array.isArray(remote) ? remote : [], chatMessages);
        if (cancelled) return;
        await api.draftChat.save(merged);
      } catch {
        // best-effort sync
      }
    })();
    return () => { cancelled = true; };
  }, [chatMessages, mockActive, draftStateHydrated]);

  React.useEffect(() => {
    if (mockActive || !draftStateHydrated) return;
    const myTeamId = user?.teamId || 1;
    try { localStorage.setItem(`fantasai_draft_queue_${myTeamId}`, JSON.stringify(queue)); } catch {}
    let cancelled = false;
    (async () => {
      try {
        const existing = await api.draftQueue.get();
        if (cancelled) return;
        const next = { ...(existing && typeof existing === 'object' ? existing : {}), [String(myTeamId)]: Array.isArray(queue) ? queue : [] };
        await api.draftQueue.save(next);
      } catch {
        // best-effort sync
      }
    })();
    return () => { cancelled = true; };
  }, [queue, mockActive, user?.teamId, draftStateHydrated]);
  ...
  }, [paused, clockSeconds, seconds, livePickNum, mockActive, draftStateHydrated]);

  // Cross-device sync: hydrate from shared R2 picks and merge into local view.
  React.useEffect(() => {
    if (mockActive) return;
    pullSharedDraftState();
    const timer = setInterval(pullSharedDraftState, 3000);
    return () => clearInterval(timer);
  }, [mockActive, pullSharedDraftState]);

  // Keep pick cursor aligned to the first unfilled slot after sync/edits.
  React.useEffect(() => {
    if (mockActive) return;
    const next = firstOpenPickNum(livePicks);
    if (next !== livePickNum) setLivePickNum(next);
  }, [livePicks, livePickNum, mockActive]);
  ...
  function joinScheduledMock(draftId, slot) {
    ...
    setMockSchedule(prev => prev.map(d => {
      ...
      if (d.status !== 'scheduled') return d;
  ...
  function leaveScheduledMock(draftId) {
    ...
    setMockSchedule(prev => {
      ...
      // Host leaving ends the mock immediately; no join should be available.
      if (target.hostTeamId === myTeamId) {
        if (activeScheduledMockId === draftId) setActiveScheduledMockId(null);
  ...
  function launchScheduledMock(draft) {
    ...
    setMockSetup(false);
    setMockActive(true);
    setActiveScheduledMockId(draft.id);
    setPaused(false);
  ...
  // playerPool is the live store list (R2 players_2026_draft/Sleeper loaded in App).
  // No local fallback list is used in Draft Room.
  const playerPool = storePlayerList;
  ...
  function attemptBigBoardDraft(playerId) {
    const myTeamId = user?.teamId || 1;
    const isMyClockTurn = isMyTurn || (!mockActive && onClockTeamId === myTeamId);
    if (isCommissioner && !mockActive && !isMyClockTurn) {
      setDraftLimitToast('Commissioner drafting from Big Board is disabled. Use Commish Concierge.');
      return false;
    }
    if (!isMyTurn) {
      const x = Math.max(0, picksAway);
      setDraftLimitToast(`You are not on the clock, ${x} picks before you`);
      return false;
    }
    if (mockActive) draftPlayer(playerId);
    else commishPick(playerId);
    return true;
  }
  ...
  if (!isLive && !mockActive) {
    ...
    const leftPanel = (
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 18, padding: 40, textAlign: 'center' }}>
        ...
        <button
          ...
          style={{ marginTop: 6, padding: '10px 18px', fontSize: 13, fontWeight: 800 }}
          onClick={startMockDraft}
        >
          🏈 Start Mock Draft
        </button>
        <div style={{ marginTop: 6, fontSize: 12, color: 'var(--text-faint)' }}>
          Mock drafts are available anytime from Draft Room.
        </div>
      </div>
    );
    return (
  ...
  return (
    ...
    >
          ...
          <div style={{
            ...
            {!mockActive && (
              <span
                style={{
                  fontSize: 11,
                  color: syncError ? '#ff8a80' : 'var(--text-faint)',
              ...
              >
                Last Sync: {formatSyncStamp(lastSyncAt)}
              </span>
            )}
            {!mockActive && (
              <button
      ...
      <div className="draft-board" style={isMobileView
        ...
        <div style={{ padding: '8px 12px', borderBottom: '1px solid var(--border)', display: 'flex', gap: 8, flexWrap: 'nowrap', alignItems: 'center' }}>
          <div className="chips">
          ...
          <input
            ...
            style={{ width: 260, minWidth: 180, maxWidth: 320 }}
          />
          {/* Hide Drafted slider */}
        ...
        {detailPlayer && (
          <InlinePlayerDetail
            ...
            onDraft={id => {
              if (attemptBigBoardDraft(id)) setDetailPlayer(null);
            }}
                  ...
                  <div style={{ marginLeft: 'auto', display: 'flex', gap: 6 }}>
                    ...
                    {(isMyTurn || (isCommissioner && !mockActive)) && (
                      <button className="btn sm primary" style={{ padding: '3px 10px', fontSize: 10, background: '#4caf82', borderColor: '#4caf82' }}
                        onClick={() => {
                          if (attemptBigBoardDraft(p.id)) {
                            setJustFreedIds(prev => {
        ...
        <div className="big-board-scroll" style={{ flex: 1, overflow: 'auto' }}>
                      ...
                      <div className="flex" style={{ gap: 4 }}>
                        ...
                        {(isMyTurn || (isCommissioner && !mockActive)) && (
                          <button
                            ...
                            onClick={() => attemptBigBoardDraft(p.id)}
      ...
      {!isMobileView && (
        <div
          ...
          title="Drag to resize Big Board and right panel"
        />
      )}
      ...
      <div className="draft-roster" style={isMobileView
        ...
        <div style={isMobileView ? { display: 'none' } : { display: 'flex', borderBottom: '1px solid var(--border)', flexShrink: 0 }}>
          {[
            ...
            <button key={tab.id}
              ...
              style={{
                flex: 1, padding: '8px 4px', fontSize: 10, fontWeight: rosterPanelView === tab.id ? 800 : 500,
          ...
          <div style={{ flex: 1, overflow: 'auto', padding: '6px 8px' }}>
            <div style={{ display: 'flex', alignItems: 'center', marginBottom: 6 }}>
              <div style={{ fontSize: 9, color: 'var(--text-faint)', letterSpacing: '.08em', textTransform: 'uppercase' }}>
                {ultraCompactTeamsView ? 'Ultra-Compact League View' : 'Compact League View'}
              </div>
              <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 6, flexShrink: 0 }}>
                <span style={{ fontSize: 9, color: ultraCompactTeamsView ? 'var(--accent)' : 'var(--text-faint)', whiteSpace: 'nowrap' }}>
                  Ultra-Compact
                </span>
                <button
                  onClick={() => setUltraCompactTeamsView(v => !v)}
                  title={ultraCompactTeamsView ? 'Disable ultra-compact view' : 'Enable ultra-compact view'}
                  style={{
                    width: 36, height: 18, borderRadius: 10, border: 'none', padding: 0,
                    background: ultraCompactTeamsView ? 'var(--accent)' : 'var(--border-strong)',
                    position: 'relative', cursor: 'pointer', flexShrink: 0,
                    transition: 'background .15s',
                  }}
                >
                  <span style={{
                    position: 'absolute', top: 2, left: ultraCompactTeamsView ? 19 : 2,
                    width: 14, height: 14, borderRadius: '50%', background: '#fff',
                    display: 'block', transition: 'left .15s',
                  }} />
                </button>
            ...
            {teamsForCols.map((t) => {
              ...
              const teamPicks = allPicks
                .filter(pk => pk.teamId === t.id && pk.playerId)
                .sort((a, b) => a.pickNum - b.pickNum);
              const picksByPos = Object.fromEntries(posOrder.map(pos => [pos, []]));
              teamPicks.forEach(pk => {
                const p = findP(pk.playerId);
                if (!p || !picksByPos[p.pos]) return;
                picksByPos[p.pos].push({ pk, p });
              });
              ...
              const canAssign = isCommissioner && !mockActive;
              ...
              return (
                <div
                  ...
                  style={{
                    ...
                    borderRadius: 6,
                    marginBottom: 6,
                    background: t.id === onClockTeamId
                      ? 'rgba(76,175,130,.08)'
                      : t.me
                        ? 'rgba(198,255,58,.05)'
                        : 'rgba(255,255,255,.01)',
                    display: 'grid',
                    gridTemplateColumns: ultraCompactTeamsView ? '132px 1fr' : '170px 1fr',
                    alignItems: 'stretch',
                    minHeight: ultraCompactTeamsView ? 30 : 38,
                  }}
                >
                  <div style={{ padding: ultraCompactTeamsView ? '3px 4px' : '5px 6px', borderRight: '1px solid var(--border)', display: 'flex', flexDirection: 'column', gap: ultraCompactTeamsView ? 2 : 3 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: ultraCompactTeamsView ? 3 : 5 }}>
                      <span style={{ fontSize: ultraCompactTeamsView ? 11 : 12 }}>{t.logo}</span>
                      <span style={{ fontSize: ultraCompactTeamsView ? 9 : 10, fontWeight: 800, color: t.id === onClockTeamId ? '#4caf82' : 'var(--text)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                        {t.name}
                      </span>
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: ultraCompactTeamsView ? 3 : 5 }}>
                      {t.id === onClockTeamId && <span style={{ fontSize: ultraCompactTeamsView ? 7 : 8, fontWeight: 900, color: '#4caf82', letterSpacing: '.08em' }}>ON CLOCK</span>}
                      {mode !== 'manual' && (
                        <span style={{ fontSize: ultraCompactTeamsView ? 7 : 8, fontWeight: 800, color: mode === 'ai' ? 'var(--accent-2)' : 'var(--accent)' }}>
                          {mode === 'ai' ? 'AI' : 'AUTO'}
                        </span>
                      )}
                      <span style={{ fontSize: ultraCompactTeamsView ? 7 : 8, color: teamGradeColor, fontWeight: 800 }}>{teamGradeLabel}</span>
                      <span style={{ marginLeft: 'auto', fontSize: ultraCompactTeamsView ? 7 : 8, color: 'var(--text-faint)', fontFamily: 'var(--font-mono)' }}>{teamPicks.length}</span>
                      {canAssign && (
                        <button
                          className="btn ghost sm"
                          style={{ fontSize: ultraCompactTeamsView ? 7 : 8, padding: ultraCompactTeamsView ? '0 2px' : '0 4px', lineHeight: 1.4 }}
                  ...
                  <div style={{
                    padding: ultraCompactTeamsView ? '3px 4px' : '4px 6px',
                    display: 'grid',
                    gridTemplateColumns: 'repeat(6, minmax(0, 1fr))',
                    gap: ultraCompactTeamsView ? 3 : 6,
                    alignContent: 'start',
                  }}>
                    {posOrder.map(pos => {
                      const posColorVar = pos === 'DST'
                        ? 'var(--pos-dst)'
                        : pos === 'K'
                          ? 'var(--pos-k)'
                          : `var(--pos-${String(pos).toLowerCase()})`;
                      const picksForPos = picksByPos[pos] || [];
                      return (
                        <div key={`${t.id}-${pos}`} style={{ minWidth: 0 }}>
                          <div style={{
                            fontSize: ultraCompactTeamsView ? 7 : 8,
                            fontWeight: 800,
                            letterSpacing: '.06em',
                            color: posColorVar,
                            marginBottom: 2,
                          ...
                          <div style={{ display: 'flex', flexDirection: 'column', gap: ultraCompactTeamsView ? 2 : 3 }}>
                            {picksForPos.length === 0 ? (
                              <span style={{ fontSize: ultraCompactTeamsView ? 7 : 8, color: 'var(--text-faint)' }}>-</span>
                            ) : picksForPos.map(({ pk, p }) => {
                              ...
                              return (
                                <span
                                  key={pk.pickNum}
                                  title={`#${pk.pickNum} R${round}.${pk.slot ?? ((pk.pickNum - 1) % 12) + 1} · ${p.name} (${p.pos})`}
                                  style={{
                                    fontSize: ultraCompactTeamsView ? 8 : 9,
                                    fontFamily: 'var(--font-mono)',
                                    color: posColorVar,
                                    border: `1px solid ${posColorVar}`,
                                    borderRadius: 4,
                                    padding: ultraCompactTeamsView ? '0 3px' : '1px 4px',
                                ...
                                >
                                  {ultraCompactTeamsView ? initial : last}
                                </span>
                              );
                            })}
                          </div>
        ...
        {activePanelView === 'ghosts' && (
          <div style={{ display: 'flex', flexDirection: 'column', flex: 1, overflow: 'hidden' }}>
            ...
            {[
              [
                { pick: currentPickNum, team: onClockTeam, teamId: onClockTeamId, isCurrent: true, isMe: isMyTurn },
                ...upcoming.slice(0, 2).map(u => ({ pick: u.pick, team: u.team, teamId: u.teamId, isCurrent: false, isMe: !!u.team?.me }))
              ],
              upcoming.slice(2, 5).map(u => ({ pick: u.pick, team: u.team, teamId: u.teamId, isCurrent: false, isMe: !!u.team?.me }))
            ].map((row, rowIdx) => (
              <div key={rowIdx} style={{ display: 'flex', gap: 8, padding: rowIdx === 0 ? '0 12px 6px' : '0 12px 10px' }}>
                {row.map(u => {
                  const ghostPreds = u.isCurrent ? currentPrediction : predictPicks(u.teamId, u.pick, draftedIds, 3);
                  return (
                    <div key={u.pick} className={`ghost-card ${u.isMe ? 'me' : ''}`} style={{ minWidth: 0, flex: 1 }}>
                      <div className="ghost-head sm">
                        ...
                        {u.isCurrent && <span className="ghost-now">ON CLOCK</span>}
                      </div>
                      <div className="ghost-targets sm">
                        {ghostPreds.map((g, i) => (
                          <GhostTarget key={g.player.id} g={g} rank={i} compact isMe={u.isCurrent ? isMyTurn : false} />
                        ))}
                      </div>
                    </div>
                  );
        ...
        {activePanelView === 'commish' && isCommissioner && (
          <div style={{ display: 'flex', flexDirection: 'column', flex: 1, overflow: 'auto', padding: '10px 12px', gap: 10, background: 'linear-gradient(180deg, rgba(255,180,0,.06) 0%, rgba(255,180,0,.02) 100%)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <span style={{ fontSize: 12, fontWeight: 900, color: '#ffb547', letterSpacing: '.06em', textTransform: 'uppercase' }}>Commish Concierge</span>
            </div>
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
              <button className="btn ghost sm" onClick={() => { const next = !paused; setPaused(next); logCommish(next ? 'pause' : 'resume', next ? 'Commissioner paused the draft clock' : 'Commissioner resumed the draft clock'); }}>
                {paused ? '▶ Resume' : '⏸ Pause'}
              </button>
              <button className="btn ghost sm" onClick={() => { setSeconds(clockSeconds); setPaused(false); }}>↺ Reset Clock</button>
              <select className="input" style={{ fontSize: 12, padding: '2px 6px', width: 90 }} value={clockSeconds} onChange={e => { const v = Number(e.target.value); setClockSeconds(v); setSeconds(v); logCommish('clock', `Commissioner set pick clock to ${v < 60 ? `${v}s` : `${v / 60}m`}`); }}>
                {[30, 60, 90, 120, 180, 300].map(s => <option key={s} value={s}>{s < 60 ? `${s}s` : `${s/60}m`}</option>)}
              </select>
            </div>
            <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
              <button className="btn ghost sm" onClick={() => removeLastPicks(1)}>↩ Undo Last</button>
              {selectedPicks.size > 0 ? (
                <button className="btn sm" style={{ fontSize: 12, background: '#ff9800', color: '#000', borderColor: '#ff9800' }} onClick={() => reverseSelectedPicks()}>
                  ↩ Reverse {selectedPicks.size}
                </button>
              ) : <span style={{ fontSize: 11, color: 'var(--text-faint)' }}>Select picks in activity log to reverse</span>}
            </div>
            {(() => {
              const pickTeamId = commishPickTeamId ?? onClockTeamId;
              const pickTeam = LEAGUE_TEAMS.find(t => t.id === pickTeamId);
              const searchPool = mockActive ? playerPool.filter(p => !new Set(mockPicks.map(x => x.playerId)).has(p.id)) : bestAvail;
              const matches = commishPickSearch.trim() ? searchPool.filter(p => p.name.toLowerCase().includes(commishPickSearch.toLowerCase())).slice(0, 7) : [];
              function handleDraftOnBehalf(p) {
                if (mockActive) commishMockPick(p.id, pickTeamId);
                else { commishPick(p.id, pickTeamId); setPaused(false); }
                setCommishPickSearch('');
                setCommishPickTeamId(null);
                logCommish('pick', `Commissioner drafted ${p.name} (${p.pos}) for ${pickTeam?.name}`);
              }
              return (
                <div style={{ position: 'relative', display: 'flex', gap: 6, alignItems: 'center', flexWrap: 'wrap' }}>
                  <span style={{ fontSize: 11, fontWeight: 700, color: 'var(--accent-2)', textTransform: 'uppercase', letterSpacing: '.07em' }}>Draft On Behalf</span>
                  <select className="input" style={{ fontSize: 12, padding: '2px 4px', width: 130 }} value={pickTeamId} onChange={e => setCommishPickTeamId(Number(e.target.value))}>
                    {LEAGUE_TEAMS.map(t => <option key={t.id} value={t.id}>{t.logo} {t.name}</option>)}
                  </select>
                  <input className="input" style={{ fontSize: 12, padding: '2px 6px', width: 180 }} placeholder="Search player..." value={commishPickSearch} onChange={e => setCommishPickSearch(e.target.value)} autoComplete="off" />
                  {matches.length > 0 && (
                    <div style={{ position: 'absolute', top: '100%', left: 0, zIndex: 60, background: 'var(--panel-2)', border: '1px solid var(--border-strong)', borderRadius: 6, minWidth: 320, boxShadow: '0 6px 24px rgba(0,0,0,.6)', overflow: 'hidden' }}>
                      <div style={{ padding: '5px 10px', fontSize: 11, fontFamily: 'var(--font-mono)', color: 'var(--accent-2)', background: 'rgba(78,168,255,.08)', borderBottom: '1px solid var(--border)' }}>{pickTeam?.logo} {pickTeam?.name} · Pick #{currentPickNum}{mockActive ? ' (mock)' : ''}</div>
                      {matches.map((p, i) => (
                        <div key={p.id} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '7px 10px', cursor: 'pointer', borderBottom: i < matches.length - 1 ? '1px solid var(--border)' : 'none' }} onMouseEnter={e => e.currentTarget.style.background = 'var(--hover)'} onMouseLeave={e => e.currentTarget.style.background = ''} onClick={() => handleDraftOnBehalf(p)}>
            ...
            <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
              {resetDraftConfirm2 ? (
                <>
                  <span style={{ fontSize: 11, color: '#ff5a6e' }}>Absolutely sure?</span>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', flex: 1, overflow: 'hidden' }}>
            <div style={{ flex: 1, overflow: 'auto', padding: '8px 0' }}>
              <div style={{ padding: '6px 12px 8px', borderBottom: '1px solid var(--border)', marginBottom: 8 }}>
                <div style={{ display: 'flex', alignItems: 'center', marginBottom: 6 }}>
                  <div className="card-title" style={{ flex: 1, fontSize: 12 }}>Chat &amp; Activity</div>
                ...
                {(() => {
                  ...
                  chatMessages.forEach((m, i) => {
                    rows.push({ kind: 'chat', key: `chat-${i}`, at: Number(m.at) || 0, msg: m });
                  });

                  const pickMap = new Map();
                  allPicks.filter(pk => pk.playerId).forEach(pk => pickMap.set(pk.pickNum, pk));
                  pickMap.forEach(pk => {
                    const at = pk.pickedAt ? Date.parse(pk.pickedAt) : (pk.pickNum || 0) * 1000;
                    rows.push({ kind: 'pick', pickNum: pk.pickNum, at: Number.isFinite(at) ? at : 0, pk });
                  });

                  commishLog.forEach(entry => {
                    rows.push({ kind: 'event', pickNum: entry.pickNum ?? 0, at: Number(entry.id) || 0, entry });
                  });

                  reversalLog.forEach((entry, i) => {
                    rows.push({ kind: 'reversal', pickNum: entry.pickNum ?? 0, at: ((entry.pickNum ?? 0) * 1000) + i + 500, i, entry });
                  });

                  rows.sort((a, b) => (b.at - a.at) || ((b.pickNum ?? 0) - (a.pickNum ?? 0)));

                  if (!rows.length) {
                    return <div className="empty">No chat or draft activity yet.</div>;
                  }
                  ...
                  return rows.slice(0, 80).map(row => {
                    if (row.kind === 'chat') {
                      const m = row.msg;
                      return (
                        <div key={row.key} className="chat-msg" style={m.ai ? { borderLeft: '2px solid var(--accent)', background: 'rgba(198,255,58,.04)' } : {}}>
                    ...
                    if (row.kind === 'event') {
                      ...
                      return (
                        <div key={`cl-chat-${entry.id}`} style={{ padding: '6px 8px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: 8, background: entryBg, borderLeft: `3px solid ${entryColor}` }}>
                    ...
                    if (row.kind === 'reversal') {
                      ...
                      return (
                        <div key={`rev-chat-${row.i}`} style={{ padding: '6px 8px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: 8, background: 'rgba(255,152,0,.07)', borderLeft: '3px solid #ff9800' }}>
                          <span style={{ fontSize: 16, flexShrink: 0 }}>↩</span>
                          <div style={{ flex: 1, minWidth: 0 }}>
                            <div style={{ color: '#ff9800', fontWeight: 700, fontSize: 11 }}>Commissioner reversed {entry.round}.{entry.slot?.toString().padStart(2, '0')}</div>
                            <div style={{ color: 'var(--text-faint)', fontSize: 10 }}>{rp.name} removed from {rt.logo} {rt.name} · {entry.reversedAt}</div>
                          </div>
                        </div>
                      );
                    }
                    ...
                    return (
                      <div key={`pick-chat-${pk.pickNum}`} style={{ padding: '7px 8px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: 8, fontSize: 12, background: isSelected ? 'rgba(255,152,0,.13)' : t.me ? 'rgba(198,255,58,.10)' : '', borderLeft: t.me && !isSelected ? '3px solid var(--accent)' : isSelected ? '3px solid #ff9800' : '3px solid transparent' }}>
                        {isCommissioner && (
                          <input
                            type="checkbox"
                            checked={isSelected}
                            onChange={() => setSelectedPicks(prev => {
                              if (!isCommissioner) return prev;
                              const n = new Set(prev);
                              n.has(pk.pickNum) ? n.delete(pk.pickNum) : n.add(pk.pickNum);
                              return n;
                            })}
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 5, flexWrap: 'nowrap', overflow: 'hidden' }}>
                            ...
                            <span style={{ fontSize: 10, fontWeight: 700, color: deltaColor, flexShrink: 0, marginLeft: 'auto' }}>{adpDelta >= 0 ? '+' : ''}{adpDelta.toFixed(1)}</span>
                          </div>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                            <span className="player-meta" style={{ color: t.me ? 'var(--accent)' : 'var(--text-dim)', fontSize: 10 }}>→ {t.logo} {t.name}</span>
                            {ghost && (ghostMatch ? <span style={{ fontSize: 9, color: '#4caf82', fontWeight: 700, marginLeft: 'auto', flexShrink: 0 }}>✓ Ghost</span> : <span style={{ fontSize: 9, color: 'var(--text-dim)', marginLeft: 'auto', flexShrink: 0 }}>Ghost: {ghost.player.name.split(' ').slice(-1)[0]}</span>)}
                          </div>
                        </div>
                      </div>
                    );
...
function DraftRecap({ round, picks = [], teamCount = 12, onClose }) {
  const roundPicks = (Array.isArray(picks) ? picks : []).filter(p => p.round === round && p.playerId);
  const grades = LEAGUE_TEAMS.map(t => {
    const myPick = roundPicks.find(p => p.teamId === t.id);
      ...
      <div style={{ position: 'fixed', inset: '60px', background: 'var(--bg-2)', border: '1px solid var(--border)', borderRadius: 12, padding: 24, overflow: 'auto', zIndex: 60 }} onClick={e => e.stopPropagation()}>
        ...
        <table className="data-table">
          ...
          <tbody>
            {roundPicks.length === 0 && (
              <tr>
                <td colSpan={7} className="dim" style={{ textAlign: 'center', padding: '20px 12px' }}>
                  No picks recorded for Round {round} yet.
                </td>
              </tr>
            )}
            {grades.filter(g => g.player).sort((a, b) => a.pickNum - b.pickNum).map(g => (
              <tr key={g.team.id}>
                <td className="mono">{Math.ceil(g.pickNum / teamCount)}.{((g.pickNum - 1) % teamCount) + 1}</td>

...
// Tier labels for the 0-100 Offensive Ecosystem scores (QB Support Score, player
// Weapon Score) — a separate scale from TIER_STYLE above, which only covers the
// unrelated rank-cutoff-based seasonTier/PlayerCell system.
...
export const SCORE_TIER_STYLE = {
  Elite:           { color: '#FFD700', bg: 'rgba(255,215,0,.15)', border: 'rgba(255,215,0,.4)' },
  Good:            { color: '#1affa0', bg: 'rgba(26,255,160,.12)', border: 'rgba(26,255,160,.35)' },
  Average:         { color: 'var(--accent-2)', bg: 'rgba(100,180,255,.12)', border: 'rgba(100,180,255,.3)' },
  'Below Average': { color: '#f5d130', bg: 'rgba(245,209,48,.12)', border: 'rgba(245,209,48,.3)' },
  Poor:            { color: '#ff4f4f', bg: 'rgba(255,79,79,.12)', border: 'rgba(255,79,79,.3)' },
};
...
export function SeasonStatBar({ label, val, max, rank, leagueN, leagueAvg }) {
  ...
  return (
    <div style={{ marginBottom: 10 }}>
      <div className="flex" style={{ justifyContent:'space-between', alignItems: 'center', marginBottom: 3 }}>
        <span className="dim" style={{ fontSize: 11 }}>{label}</span>
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          {rank != null && leagueN != null && (
            <span style={{ fontSize: 9, fontFamily: 'var(--font-mono)', color: rankColor, fontWeight: 700, letterSpacing: '.04em' }}>#{rank}/{leagueN}</span>
          )}
          <span style={{ fontFamily:'var(--font-mono)', fontSize: 12, fontWeight: 600 }}>{val ?? '—'}</span>
        </div>
      </div>
      <div style={{ height: 3, background:'var(--panel-3)', borderRadius: 2, position: 'relative' }}>
        <div style={{ width:`${pct}%`, height:'100%', background: barColor, borderRadius: 2, transition:'width .4s' }} />
        {avgPct != null && (
          <div style={{ position: 'absolute', top: -3, left: `${avgPct}%`, width: 2, height: 9, background: '#ffd700', borderRadius: 1, transform: 'translateX(-50%)', opacity: 0.85 }} />
        )}
      </div>
      {leagueAvg != null && (
        <div style={{ fontSize: 9, fontFamily: 'var(--font-mono)', color: '#ffd700', marginTop: 3, letterSpacing: '.04em', opacity: 0.85 }}>
          Lg Avg: {fmtAvg(leagueAvg)}
        </div>
      )}
    </div>
  );
...
// Raw-SVG radar/spider chart — no chart library, matches Sparkline's style above.
// axes: [{ label, value }] where value is 0-100 or null (renders as 0 on the chart,
// but callers should generally omit an axis entirely if the underlying score is null).
export const RadarChart = ({ axes, size = 220, color = 'var(--accent)' }) => {
  const cx = size / 2, cy = size / 2, r = size / 2 - 28;
  const angleFor = i => (Math.PI * 2 * i) / axes.length - Math.PI / 2;
  const pointFor = (i, val) => {
    const a = angleFor(i), rad = (Math.max(0, val ?? 0) / 100) * r;
  ...
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
      {[25, 50, 75, 100].map(lvl => (
        <polygon key={lvl} fill="none" stroke="var(--border)" strokeWidth="1" opacity={0.5}
      ...
      <polygon points={dataPoints.map(p => p.join(',')).join(' ')} fill={color} fillOpacity="0.25" stroke={color} strokeWidth="2" />
      {dataPoints.map(([x, y], i) => <circle key={i} cx={x} cy={y} r="3" fill={color} />)}
      {axes.map((ax, i) => {
        const [x, y] = pointFor(i, 124);
        return <text key={i} x={x} y={y} fontSize="9" fill="var(--text-dim)" textAnchor="middle">{ax.label}</text>;
      })}
    </svg>
  );
};

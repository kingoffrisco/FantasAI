...
def main():
    ...
    if not args.skip_stats:
        ...
        # Offensive Ecosystem (QB Support Score + Player Weapon Score — depends on
        # team_oline_index above being fresh)
        eco_args = extra[:]
        if args.seasons:
            eco_args += ["--seasons", args.seasons]
        if not run("ingest/ingest_offensive_ecosystem.py", eco_args, "Offensive Ecosystem — Support Score + Weapon Score"):
            failed.append("offensive_ecosystem")

        # Depth Chart History (real per-week O-Line starting lineups, 2021-2025 —
        # feeds the O-Line Stability Index below)
        dch_args = extra[:]
        if args.seasons:
            dch_args += ["--seasons", args.seasons]
        if not run("ingest/ingest_depth_chart_history.py", dch_args, "Depth Chart History — real per-week O-Line lineups"):
            failed.append("depth_chart_history")

        # O-Line Stability Index (continuity, chemistry, health, experience,
        # penalties — depends on depth_chart_history + team_oline_index above)
        olsi_args = extra[:]
        if args.seasons:
            olsi_args += ["--seasons", args.seasons]
        if not run("ingest/ingest_oline_stability.py", olsi_args, "O-Line Stability Index — continuity + chemistry"):
            failed.append("oline_stability")

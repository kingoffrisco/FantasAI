...
export default function OwnerIntelScreen({ onOpenPlayer, user, myRosterIds, slotOverrides }) {
  ...
  const [mobileTab, setMobileTab] = React.useState('owners');
  ...
  const ownerList = (
    <div className="intel-rail" style={isMobile ? { height: '100%', overflow: 'auto' } : undefined}>
  ...
  const profileView = (
    <div className="intel-main" style={isMobile ? { height: '100%', overflow: 'auto' } : undefined}>
      {selected && team && <OwnerProfile key={selectedId} owner={selected} team={team} onOpenPlayer={onOpenPlayer} isMyTeam={selectedId === user?.teamId} myRosterIds={myRosterIds} slotOverrides={slotOverrides} commissioners={commissioners} selectedId={selectedId} isMobile={isMobile} mobileTab={mobileTab} />}
    </div>
  );
  ...
  return (
    <div className="col" style={{ height: '100%', overflow: 'hidden' }}>
      ...
      {isMobile && (
        <div style={{ display: 'flex', gap: 0, borderBottom: '1px solid var(--border)', background: 'var(--bg-2)', overflowX: 'auto', whiteSpace: 'nowrap' }}>
          {[
            ...
            <button
              ...
              style={{
                ...
                padding: '10px 8px',
                fontSize: 11,
                fontWeight: mobileTab === t.id ? 700 : 500,
                border: 'none',
      ...
      {isMobile ? (
        <div style={{ flex: 1, minHeight: 0 }}>
          {mobileTab === 'owners' ? ownerList : profileView}
        </div>
      ) : (
...
function OwnerProfile({ owner, team, onOpenPlayer, isMyTeam, myRosterIds, slotOverrides, commissioners, selectedId, isMobile = false, mobileTab = 'profile' }) {
  const o = owner;
  const realPosSpend = React.useMemo(() => computePosSpend(team.id), [team.id]);
  const posData = realPosSpend || o.posByRound;

  const showProfile = !isMobile || mobileTab === 'profile';
  const showTendencies = isMobile && mobileTab === 'tendencies';
  const showRoster = isMobile && mobileTab === 'roster';
  const showSpend = !isMobile || mobileTab === 'spend';
  const showPrediction = !isMobile || mobileTab === 'prediction';
  const showHistory = !isMobile || mobileTab === 'history';
  const showPlaybook = !isMobile || mobileTab === 'playbook';
  ...
  return (
    <div className="intel-profile">
      ...
      {showTendencies && (
      <div className="muted-card" style={{ marginTop: isMobile ? 10 : 0 }}>
        <div className="card-mini-label">DRAFT TENDENCIES</div>
        <div className="metric-grid">
          <Metric label="Reach (vs ADP)" value={`${o.metrics.reach > 0 ? '+' : ''}${o.metrics.reach.toFixed(1)}`} unit="picks" color={o.metrics.reach > 5 ? 'var(--danger)' : o.metrics.reach > 2 ? 'var(--warn)' : 'var(--good)'} />
          <Metric label="Predictability" value={o.metrics.predictability} unit="%" color={o.metrics.predictability > 80 ? 'var(--good)' : 'var(--warn)'} />
      ...
      {showPlaybook && o.you && (
        <div className="muted-card" style={{ marginTop: 14, marginBottom: 14 }}>
          <div className="card-mini-label">PLAYBOOK</div>
          <div className="dim" style={{ fontSize: 12 }}>Playbook tips are shown only for other owners.</div>
        </div>
      )}
    </div>
  );
